  ...
  return (
    <>
      ...
      <div
        className={`
          fixed lg:relative
          w-64 bg-white flex flex-col h-full shadow-md
          z-30 transition-transform duration-300 ease-in-out
          ${isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}
        `}
      >
      <div className="p-4">
        <h2 className="text-lg font-semibold text-secondary-900">CV Sections</h2>
        <p className="text-xs text-secondary-500 mt-1">Drag to reorder</p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        {sections.map((section) => (
          <div
            ...
            className={`
              flex items-center gap-2 px-4 py-3 cursor-pointer border-b border-secondary-100
              transition-colors
              ${activeSection === section.id ? 'bg-primary-50 border-l-4 border-l-primary-600' : 'hover:bg-secondary-50'}
              ${draggedItem === section.id ? 'opacity-50' : ''}
            `}
          >
            <GripVertical className="w-4 h-4 text-secondary-400 flex-shrink-0" />

            <span className={`flex-1 text-sm font-medium ${section.visible ? 'text-secondary-900' : 'text-secondary-400'}`}>
              {section.title}
            </span>
